...
[ -z "${AWS_ACCESS_KEY_ID-x}" ] || [ -z "${AWS_SECRET_ACCESS_KEY-x}" ] && "AWS access key and secret must be set.. exiting." && exit 1
...
# test aws-cli
sudo docker run --rm -e AWS_ACCESS_KEY_ID="${AWS_ACCESS_KEY_ID}" -e AWS_SECRET_ACCESS_KEY="${AWS_SECRET_ACCESS_KEY}" aws-cli -- aws sts get-caller-identity > /dev/null 2>&1
